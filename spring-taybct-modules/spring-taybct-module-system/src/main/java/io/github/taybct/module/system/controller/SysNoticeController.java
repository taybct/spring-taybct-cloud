 ...
 */
@Tag(name = "消息通知相关接口")
@RequestMapping(ServeConstants.CONTEXT_PATH_SYSTEM + "{version}/notice")
@ApiVersion
@RestController
@RequiredArgsConstructor
public class SysNoticeController implements BaseController<SysNotice, ISysNoticeService> {
    ...
    @WebLog
    public R<IPage<SysNoticeVO>> userNoticesPage(@RequestParam(required = false) Map<String, Object> sqlPageParams) {
        return R.data(getBaseService().userNoticesPage(sqlPageParams));
    }
    ...
    public R<SysNotice> addRelatedNotices(@Valid @NotNull @RequestBody SysNoticeDTO dto) {
    ...
    public R<?> updateUserNotices(@Valid @NotNull @PathVariable Integer status, @Valid @NotNull @RequestBody Collection<Long> noticeIds) {
